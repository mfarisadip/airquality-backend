...
export const AuthController = new Elysia()
  ...
  .post(
    ...
    async ({ body, cookie: { access_token_cookie }, set, jwt }) => {
      ...
      // set access token to cookie using new syntax
      access_token_cookie.set({
        value: accessToken,
        httpOnly: true,
        maxAge: 2 * 7200, // 2 hours
        path: '/' // tambahkan path jika diperlukan
      });
  ...
  .post("logout", async ({ cookie: { access_token_cookie }, set }) => {
    // remove cookie using new syntax
